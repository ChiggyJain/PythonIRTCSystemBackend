        ...
        while True:
            ...
            for loopNo in range(1, BATCH_SIZE):
                ...
                async with AsyncSessionLocal() as db:
                    async with db.begin():
                        ...
                        events = await outbox_repo.fetch_pending_outbox_events(
                            event_type="EMAILVERIFICATION_OTP_DISPATCH_REQUESTED", limit=1, now_time=now_ist(),
                        )
                        if not events:
                            break
                        event = events[0]
                ...
                try:
                    ...
                    # kafka topic
                    topic = settings.KAFKA_EMAILVERIFICATION_OTP_DISPATCH_TOPIC
                    ...
                    async with AsyncSessionLocal() as db:
                        async with db.begin():
                            ...
                            await security_repo.add_security_event(
                                user_id=user_id,
                                event_name="email_verification_outbox_published",
                    ...
                    async with AsyncSessionLocal() as db:
                        async with db.begin():
                            ...
                            if event!=None:
                                params = {
                                    "retry_handler_type": "EMAILVERIFICATION_OTP", "outbox_repo": outbox_repo, "security_repo": security_repo
                                }
                                emailverification_otp_outbox_retry_handler_class_obj = OutboxRetryHandlerFactory.getOutboxRetryHandler(**params)
                                params = {
                                    "event": event, "user_id": user_id, "error_message": str(exc)
                                }
                                await emailverification_otp_outbox_retry_handler_class_obj.handle(**params)
