...
async def run_worker() -> None:
    ...
    producer = build_producer(
        client_id=f"{settings.KAFKA_CLIENT_ID}-schedule-inventory-seat-availability-updated-outbox-publisher"
    )
    await producer.start()
    app_logger.info("schedule_inventory_seat_availability_updated_outbox_worker started")
        ...
        while True:
            ...
            for loopNo in range(1, BATCH_SIZE):
                ...
                try:
                    ...
                    key = str(payload.get("schedule_id", 0)).encode("utf-8")
                    ...
                    async with AsyncSessionLocal() as db:
                        async with db.begin():
                            ...
                            if event!=None:
                                params = {
                                    "retry_handler_type": "SCHEDULE_INVENTORY_SEAT_AVAILABILITY_UPDATED", "outbox_repo": outbox_repo
                                }
