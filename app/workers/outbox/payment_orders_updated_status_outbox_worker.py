...
async def run_worker() -> None:
    ...
    producer = build_producer(
        client_id=f"{settings.KAFKA_CLIENT_ID}-payment-order-updated-status-outbox-publisher"
    )
    await producer.start()
    app_logger.info("payment_orders_updated_status_outbox_worker started")
                        ...
                        async with db.begin():
                            ...
                            if db_event:
                                retry_handler = OutboxRetryHandlerFactory.getOutboxRetryHandler(
                                    retry_handler_type="PAYMENT_ORDERS_UPDATED_STATUS",
